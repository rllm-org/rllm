    ...
    def receive(self, session_id, current_url, session_int=None, **kwargs):
        ...
        with app.app_context(), app.test_request_context():
            ...
            if session_id not in self.user_sessions:
                if not self.goals:
                    raise ValueError(
                        "WebShop SimServer has no goals loaded. "
                        "Check `filter_goals`/`limit_goals` settings and WebShop data files."
                    )

                # `session_int` is often a global trajectory/session id (e.g. 0..N-1). When
                # `limit_goals` is set smaller than N, indexing directly will crash. We keep
                # deterministic behavior by wrapping via modulo.
                ...
                self.user_sessions[session_id] = {'goal': goal, 'goal_idx': idx, 'done': False}
            else:
                instruction_text = \
