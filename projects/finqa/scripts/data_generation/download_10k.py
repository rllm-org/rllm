...
def load_sec_content(url, user_agent):
    ...
    try:
        ...
        iframe = WebDriverWait(driver, 30).until(EC.presence_of_element_located((By.ID, "ixvFrame")))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Download SEC 10-K tables via headless Chrome")
    parser.add_argument("--user_agent", type=str, required=True, help="SEC-required email address for User-Agent header")
    parser.add_argument("--seed_urls_path", type=str, default=SCRAPED_DATA_URLS_PATH, help="CSV with 'url' and 'output_directory' columns")
    parser.add_argument("--output_base_dir", type=str, default=None, help="Output directory (default: scraped_data/<date>)")
    parser.add_argument("--max_workers", type=int, default=10, help="Parallel downloads (default: 10)")
    args = parser.parse_args()

    output_base_dir = args.output_base_dir or os.path.join(SCRAPED_DATA_DIR, datetime.now().strftime("%Y-%m-%d"))
    os.makedirs(output_base_dir, exist_ok=True)

    df = pd.read_csv(args.seed_urls_path)
    print(f"Processing {len(df)} companies with {args.max_workers} workers...\n")

    results = []
    with ThreadPoolExecutor(max_workers=args.max_workers) as executor:
        futures = {executor.submit(process_company, row, output_base_dir, args.user_agent, idx, len(df)): idx for idx, row in df.iterrows()}
