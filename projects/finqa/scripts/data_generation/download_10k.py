...
def load_sec_content(url, user_agent):
    ...
    )  # This follows the SEC's required format
    ...
    try:
        ...
        for attempt in range(5):
            ...
            if len(valid_tag_names) > 0:
                ...
                print(
                    f"[{index + 1}/{total}] {output_dir}: Found 0 tables, retrying..."
                )
                rendered_html = load_sec_content(url, user_agent)
                soup = BeautifulSoup(rendered_html, "html.parser")

        print(
            f"[{index + 1}/{total}] {output_dir}: Found {len(valid_tag_names)} tables"
        )

        for tag_name in valid_tag_names:
            element = remove_attributes(
                soup.find("ix:nonnumeric", attrs={"name": tag_name})
            )
        ...
        print(
            f"[{index + 1}/{total}] {output_dir}: ✓ Saved {len(valid_tag_names)} tables"
        )
        return {
            "company": output_dir,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--user_agent",
        type=str,
        help="The user agent to use for the browser, this should be an email address.",
    )
    parser.add_argument(
        "--seed_urls_path",
        type=str,
        default=SCRAPED_DATA_URLS_PATH,
        help="The path to the CSV file that contains the seed URLs. The CSV should have two columns: url and output_directory. Default is the seed URLs file in the scraped data directory.",
    )
    parser.add_argument(
        "--output_base_dir",
        type=str,
        default=None,
        help="Output directory. If not provided, the output will be saved in the scraped data directory with the date as the subdirectory.",
    )
    parser.add_argument(
        "--max_workers",
        type=int,
    ...
    print(
        f"Processing {len(df)} companies with {args['max_workers']} parallel workers...\n"
    )
    ...
    with ThreadPoolExecutor(max_workers=args["max_workers"]) as executor:
        futures = {
            executor.submit(
                process_company, row, output_base_dir, args["user_agent"], idx, len(df)
            ): idx
