...
def test_request_to_normalized_string_prompt():
    ...
    assert req.kwargs == {"temperature": 0.5}
...
def test_outbound_nonstream():
    resp = NormalizedResponse(text="hello", finish_reason="length", usage=Usage(prompt_tokens=2, completion_tokens=3))
...
async def test_outbound_stream():
    resp = NormalizedResponse(text="abc", usage=Usage(prompt_tokens=1, completion_tokens=3))
    chunks = []
