...
**P1-6. Ephemeral sandboxes (default `ephemeral=True`, `auto_delete_interval=0`).** ✅ implemented
RL rollout/warm-queue sandboxes are strictly single-use, so default to ephemeral — matching how harbor's Daytona env always runs (`environment.py:1125`, where `ephemeral=True` ⟺ `auto_delete_interval=0`). Two payoffs: (1) Daytona's **ephemeral tier carries a higher per-sandbox quota** (disk/cpu/mem), so tasks declaring more than the standard 10 GB disk — tmax's 20 GB — schedule instead of `400`-ing with "Disk request exceeds maximum allowed". This is the right fix for the disk ceiling, *not* a clamp that diverges from the task's declared (Harbor) resources. (2) **Leak-proofing** — an ephemeral sandbox is *deleted* (not left stopped) when it stops, so a SIGKILL'd run that never reaches `close()`/atexit no longer strands a stopped box forever (the ~1.3 TB of leaked stopped sandboxes observed on the shared account). Overridable via the `ephemeral` kwarg. (`daytona.py` `__init__`.)
