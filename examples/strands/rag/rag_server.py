...
class BatchingRetriever:
    ...
    async def _process_batch(self, batch: list[PendingQuery]):
        ...
        async with self.processing_lock:
            ...
            try:
                ...
                queue_depth = len(self.pending_queries)
                print(f"Batch {self.total_batches}: {batch_size} queries in {batch_time * 1000:.1f}ms ({avg_latency:.1f}ms/query, {throughput:.1f} q/s) concurrent={self.concurrent_requests} queue={queue_depth}")
    ...
    async def search_async(self, query: str, k: int = 10) -> list[dict[str, Any]]:
        ...
        # Wait for result with concurrency tracking
