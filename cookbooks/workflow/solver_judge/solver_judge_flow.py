"""Solver-Judge Workflow — multi-agent countdown solver (legacy Workflow API).

A solver generates N candidate solutions in parallel, then a judge
selects the best one. Built on the ``rllm.workflows.workflow.Workflow``
base class and ``RolloutEngine`` (the API kept around for backward
compatibility with the pre-AgentFlow examples).

For the new ``@rllm.rollout`` AgentFlow-protocol version of the same
task, see ``cookbooks/solver_judge_flow/``.
"""
...
Output the index of your selected solution within <answer>...</answer> tags, e.g.,
<answer>1</answer> for the first solution, <answer>2</answer> for the second
solution, etc. If multiple solutions are correct, output the index of the first
correct solution."""
        return prompt
