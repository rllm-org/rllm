...
- strict synchronous on-policy GRPO with eight accepted prompt groups × 16
  rollouts (128 trajectories) per optimizer step
- one forward/backward accumulation pass per accepted prompt group, followed by
  one optimizer step and an awaited deployment hot-load before new rollouts
...
an otherwise useless trainer batch. The synchronous accumulator replaces it
with another prompt generated by the same frozen policy until eight useful
groups are ready. This is separate from compact filtering: it does not remove
mixed groups or identify infrastructure failures.
