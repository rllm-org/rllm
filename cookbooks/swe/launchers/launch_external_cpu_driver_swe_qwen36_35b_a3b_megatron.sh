#!/usr/bin/env bash
# Launch the SWE Qwen3.6-35B-A3B Megatron driver on a Modal-good CPU Ray head.
#
# Expected topology:
...
if [ -z "$MODEL_SNAPSHOT" ] && [ -d /tmp/hf_cache/hub/models--Qwen--Qwen3.6-35B-A3B/snapshots ]; then
...
if [ -z "$MODEL_SNAPSHOT" ] || [ ! -d "$MODEL_SNAPSHOT" ]; then
    echo "Missing Qwen3.6-35B-A3B model snapshot. Set MODEL_PATH or stage /tmp/hf_cache/hub/models--Qwen--Qwen3.6-35B-A3B." >&2
    exit 2
fi
...
export ROLLOUT_SKIP_MM_PROFILING="${ROLLOUT_SKIP_MM_PROFILING:-true}"
export ROLLOUT_DISTRIBUTED_EXECUTOR_BACKEND="${ROLLOUT_DISTRIBUTED_EXECUTOR_BACKEND:-uni}"
