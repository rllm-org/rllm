...
class TestVerlGatewayWorkers:
    ...
    def test_multiworker_preflights_and_starts_front(self, monkeypatch):
        ...
        monkeypatch.setattr("rllm.gateway.manager.preflight_gateway_port", lambda port: checked_ports.append(port))
        monkeypatch.setattr("rllm.gateway.manager.subprocess.Popen", lambda command: commands.append(command) or object())
