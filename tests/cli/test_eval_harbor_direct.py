...
def test_harbor_pair_invokes_harbor_job_without_proxy_or_gateway(tmp_path):
    dataset = Dataset(data=[{"id": "task", "instruction": "fix", "task_path": str(tmp_path / "task")}], name="swebench-verified", split="default")
    entry = {
    ...
    with (
        ...
        patch("rllm.eval.agent_loader.load_agent") as load_agent,
    ...
    assert kwargs["harbor_config"] == str(tmp_path / "harbor.json")
