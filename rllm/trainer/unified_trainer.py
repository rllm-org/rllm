    ...
    async def _evaluate_dataloader_async(
        ...
        *,
        ...
        try:
            ...
            # Canonical fixed-suite reward: every attempted episode stays in the
            # denominator, including compact-filtered infrastructure failures and
            # empty trajectories. Role-specific reward/* metrics above remain a
            # scorable-trajectory diagnostic and can therefore have a smaller
            # denominator; use this metric (or data-source pass@1) for learning
            # curves and cross-check it with the explicit counts.
            if is_correct_array.size:
                val_metrics[f"{metric_prefix}/reward/mean"] = np.mean(is_correct_array)
                val_metrics[f"{metric_prefix}/reward/num_episodes"] = int(is_correct_array.size)
                val_metrics[f"{metric_prefix}/reward/num_correct"] = int(np.sum(is_correct_array))
