    ...
    def _validate_agent(self):
        ...
        for data_source in np.unique(data_source_array):
            ...
            for prefix in termination_metric_prefixes:
                metrics[f"{prefix}/total"] = termination_total
            for reason in TerminationReason:
                count = termination_counts.get(reason.value, 0)
                for prefix in termination_metric_prefixes:
                    metrics[f"{prefix}/{reason.value}/count"] = count
                    metrics[f"{prefix}/{reason.value}/rate"] = count / termination_total if termination_total else 0.0
            for reason, count in termination_counts.items():
                if reason in known_termination_reasons:
                    continue
                for prefix in termination_metric_prefixes:
                    metrics[f"{prefix}/{reason}/count"] = count
                    metrics[f"{prefix}/{reason}/rate"] = count / termination_total if termination_total else 0.0
