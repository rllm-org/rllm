...
class VerlInferenceClient:
    ...
    async def generate(self, request: TokenInput) -> TokenOutput:
        ...
        except (self._ray.exceptions.RayActorError, self._ray.exceptions.RaySystemError) as exc:
            raise GatewayError("Verl inference service is unavailable", 503, "server_error") from exc
        ...
        if raw.log_probs is None:
            raise RuntimeError("Verl returned no completion logprobs")
        if len(raw.log_probs) != len(raw.token_ids):
