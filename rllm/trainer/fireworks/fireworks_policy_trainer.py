...
def builtin_loss_args(algorithm_config: AlgorithmConfig):
    """Map an rLLM ``AlgorithmConfig`` to the cookbook's builtin loss config."""
    ...
    return LossConfig(
        ...
        dapo=DAPOConfig(eps_clip=eps, eps_clip_high=eps_high if eps_high is not None else 0.28),
        gspo=GSPOConfig(clip_ratio_low=eps, clip_ratio_high=eps_high),
        cispo=CISPOConfig(eps_low=eps, eps_high=eps_high if eps_high is not None else 0.28),
    )
...
class FireworksPolicyTrainer:
    ...
    def resolve_builtin_loss(self, algorithm_config: AlgorithmConfig, profile=None):
        ...
        self._builtin_loss = get_builtin_loss_config(args)
    ...
    async def forward_backward_from_trajectory_groups(
        ...
        # build_builtin_loss_datums folds advantage + TIS into per-token advantages
        # (fireworks-ai 1.2.1);
