...
class EvalGatewayManager:
    ...
    def build_config(self) -> GatewayConfig:
        ...
        #
        # ``reasoning_effort`` + ``tools[]`` on ``/v1/chat/completions`` is
        # rejected by GPT-5 ("Please use /v1/responses instead"). Reasoning
        # models still reason without the explicit knob — the field only
        # tunes effort. Dropping it lets opencode/codex/etc. (which send
        # ``reasoning_effort=medium`` unconditionally) work against GPT-5
        # via the chat-completions endpoint.
        drop_params = ["max_tokens", "reasoning_effort"] if self.provider == "openai" else []
