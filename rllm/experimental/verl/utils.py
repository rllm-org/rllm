"""Small utility functions for the Verl backend that can't be imported from verl directly."""
...
def propagate_rllm_to_verl_config(config: DictConfig) -> None:
    ...
    if train_sampling.get("top_p") is not None:
        rollout.top_p = train_sampling.top_p
    ...
    if val_sampling.get("top_p") is not None:
...
def save_checkpoint(
    ...
    # Write latest checkpoint tracker (skip for async save)
    async_save = False
...
def balance_batch(
    ...
    Mutates ``batch`` in-place via ``batch.reorder()``.
    """
    ...
    # Get DP size from dispatch info
    role_key = "actor"
