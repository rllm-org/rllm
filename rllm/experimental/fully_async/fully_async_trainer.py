...
class FullyAsyncTrainer(FullyAsyncRayPPOTrainer):
    ...
    def _process_batch_common(self, batch, metrics, timing_raw, local_trigger_step=None):
        ...
        return batch, {# Copyright 2025 Meituan Ltd. and/or its affiliates
#
# Licensed under the Apache License, Version 2.0 (the "License");
# you may not use this file except in compliance with the License.
# You may obtain a copy of the License at
...
class FullyAsyncTrainer(FullyAsyncRayPPOTrainer):
    ...
    def __init__(
        ...
        # Gradient accumulation tracking for micro-batch on-policy updates
        self.gradient_accumulation_steps = config.async_training.get("gradient_accumulation_steps", 1)
        self.current_micro_batch_idx = 0
    ...
    def _process_batch_common(self, batch, metrics, timing_raw, local_trigger_step=None):
        ...
        if self.config.trainer.critic_warmup <= self.global_steps:
            ...
            with marked_timer("update_actor", timing_raw, color="red"):
                ...
                # Determine if we should skip the optimizer step based on gradient accumulation
                # Data comes in as a stream, so we track which micro-batch we're on
                if self.gradient_accumulation_steps > 1:
                    # Check if this is the last micro-batch in the accumulation window
                    is_last_micro_batch = self.current_micro_batch_idx == self.gradient_accumulation_steps - 1
                    skip_optimizer_step = not is_last_micro_batch

                    # Pass gradient accumulation parameters through meta_info
                    batch.meta_info["skip_optimizer_step"] = skip_optimizer_step
                    batch.meta_info["gradient_accumulation_steps"] = self.gradient_accumulation_steps

                    actor_output = self.actor_rollout_wg.update_actor(batch)

                    # Update micro-batch counter
                    self.current_micro_batch_idx = (self.current_micro_batch_idx + 1) % self.gradient_accumulation_steps
                else:
                    # Default behavior: single update_actor call with no gradient accumulation
                    actor_output = self.actor_rollout_wg.update_actor(batch)
