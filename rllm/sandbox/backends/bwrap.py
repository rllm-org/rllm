...
class BwrapSandbox:
    ...
    def _build_bwrap_cmd(self, command: str) -> list[str]:
        ...
        # Mount whitelisted host paths read-only
        for path in _RO_BIND_PATHS:
            if os.path.exists(path):
                cmd.extend(["--ro-bind", path, path])
        # Handle merged-usr: /bin, /lib, /sbin may be symlinks to /usr/...
        ...
        # Writable /tmp and /app backed by host workdir
        cmd.extend([
            "--tmpfs", "/root",
            "--dev", "/dev",
            "--proc", "/proc",
    ...
    def _translate_path(self, remote_path: str) -> str:
        ...
        if remote_path.startswith("/tmp"):
            ...
            return result
        elif remote_path.startswith("/app"):
