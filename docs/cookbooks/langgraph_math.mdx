...
It is intentionally a near-clone of [`cookbooks/math_tool_agent`](/cookbooks/math_tool_agent) so you can compare a hand-rolled tool loop against a LangGraph one on the same dataset.
