...
<Note>
The legacy `rllm.trainer.agent_trainer.AgentTrainer` (with `fireworks` backend
support and `agent_run_func`) is still importable but emits a
`DeprecationWarning` on instantiation. Migrate to the unified trainer via
`from rllm.trainer import AgentTrainer`.
</Note>
