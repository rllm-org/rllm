...
Modern agents use the `Evaluator` protocol — a separate function that
...
def math_evaluator(task: dict, episode: Episode) -> EvalOutput:
    ...
    truth = str(task.get("ground_truth", ""))
    is_correct = grade_answer_mathd(answer, truth) or grade_answer_sympy(answer, truth)
    return EvalOutput(
...
See [`cookbooks/math/math_eval.py`](https://github.com/rllm-org/rllm/blob/main/cookbooks/math/math_eval.py)
for the full version that this snippet is adapted from.

## Using Rewards with Workflows

For the legacy [Workflow](/api/workflows) path, the reward function is
passed in directly:
