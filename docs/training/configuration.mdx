...
| `mask_agent_timeout` | `bool` | `True` | Mask trajectories whose agent ran out of wall clock (budget exhaustion) |
| `mask_verifier_timeout` | `bool` | `True` | Mask trajectories whose verifier ran out of wall clock (infrastructure; the reward is not a measurement) |
...
`mask_timeout` (pre-split) is still accepted and sets both timeout keys unless they are given explicitly.
