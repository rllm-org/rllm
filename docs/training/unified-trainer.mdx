...
| `benchmark_dataset` | `Dataset \| None` | Optional boundary-only benchmark dataset |
...
An optional `benchmark_dataset` uses the same evaluation path but is independent
of periodic validation. Set `benchmark_before_train=true` and/or
`benchmark_after_train=true` to run it at step 0 and/or final weights. Periodic
metrics use the `val/*` namespace; boundary benchmark metrics use
`benchmark/*`. When both suites run at the same optimizer step, their metrics
are committed in one tracking row.
...
| `rllm.trainer` | *(direct)* | `total_epochs`, `total_batches`, `test_freq`, `save_freq`, validation/benchmark boundaries, logger |
