...
- one scalar advantage per trajectory; the collector expands it to one value per response token before backend transformation.

<Note>
The registry return interface remains **trajectory-level scalar in, trajectory-level scalar out**. The built-in `minus_length_weighted_mean` estimator can additionally write token targets through `Step.advantage`; arbitrary per-token or critic-based estimators should still use [Pre-computing advantage in workflow](/training/precompute-advantage).
</Note>
...
| `MINUS_LENGTH_WEIGHTED_MEAN` | `importance_sampling` |
...
| `MINUS_LENGTH_WEIGHTED_MEAN` | `minus_length_weighted_mean` | One response-length-weighted baseline across every group of a role; optional per-token targets |
...
The estimators above are intentionally a subset of Verl's full catalog. Most fit the portable **scalar reward per trajectory, scalar advantage per trajectory** contract. `minus_length_weighted_mean` is a narrow token-aware extension supported by all three unified backends.
...
### Minus length-weighted mean

With scalar trajectory rewards, the estimator computes one baseline across all
groups of the same role and stores the resulting target once per response token:

$$
b = \frac{\sum_i r_i L_i}{\sum_i L_i},
\qquad A_i = r_i - b.
$$

If every non-empty step provides a list aligned with its generated tokens:

```python
step.metadata["per_token_rewards"] = [r_1, r_2, ..., r_L]
```

the baseline and targets instead use the observed token values:

$$
b = \frac{\sum_i\sum_t r_{i,t}}{\sum_i L_i},
\qquad A_{i,t} = r_{i,t} - b.
$$

Token mode is all-or-none within a role pool. Prompt, observation, and padding
tokens remain masked; the target list covers only `step.response_ids`. The
returned scalar `r_i - b` is retained for sample-level metrics.
