...
- **No in-reward KL.** `algorithm.use_kl_in_reward=True` is rejected at startup. KL-in-loss is supported — set `actor_rollout_ref.actor.use_kl_loss=True` and it runs inside verl's actor worker.
