...
<Note>
The estimator interface is **trajectory-level scalar in, trajectory-level scalar out**. Per-token signals (returns over time, GAE, K3 detector, etc.) cannot be expressed through this hook today. For per-token signals, see [Pre-computing advantage in workflow](/experimental/precompute-advantage).
</Note>
...
- [Pre-computing advantage in workflow](/experimental/precompute-advantage) — for per-token signals (SFT-like, OPD)
- [Unified trainer](/experimental/unified-trainer) — `AlgorithmConfig` reference
- [Configuration](/experimental/configuration) — full config field list
