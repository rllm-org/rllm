...
For a fixed table of "shared keys" (the same value, different paths in the two namespaces), `sync_config` mirrors the value between the verl-native side and the `rllm.*` side at trainer startup. You can drive any of these knobs from either namespace, and the other namespace will see the same value.
...
- **`actor.use_kl_loss` is derived** from `kl_beta`: if you do not explicitly set `actor_rollout_ref.actor.use_kl_loss` on the CLI, `sync_config` sets it to `(kl_beta > 0)`. Setting the KL coefficient enables the KL-in-loss term automatically.
- **`clip_ratio` family.** `rllm.algorithm.eps_clip` mirrors to `actor_rollout_ref.actor.clip_ratio` (the symmetric fallback verl uses when `clip_ratio_low` / `clip_ratio_high` are unset). When the user sets `actor.clip_ratio_low` (or `actor.clip_ratio`) on the CLI, `rllm.algorithm.eps_clip` is updated to match. The asymmetric upper bound goes through the regular `eps_clip_high ↔ clip_ratio_high` shared-key bridge.
...
The Verl backend uses bidirectional sync (described above): for any shared key, set it on either the verl-native side or the `rllm.*` side and the other side stays in agreement. The Tinker backend does its own one-way forwarding from native group-size settings into `rllm.rollout.{n,n_val}` (see "Tinker backend" above).

### Example: pick a knob, set it on either side

You can set `adv_estimator` on the verl-native side:

```bash
python train.py rllm/backend=verl algorithm.adv_estimator=rloo
```

…or on the `rllm.*` side — both produce the same configuration:

```bash
python train.py rllm/backend=verl rllm.algorithm.adv_estimator=rloo
```

If you accidentally set both at once, the `rllm.*` value wins (per the precedence rules).
...
The verl-native equivalent is also valid:
...
- **Backward compatibility.** Existing scripts that override on the verl-native side continue to work; the rllm-side value is mirrored automatically.
