...
Every one of these parses through the typed readers in `rllm/env.py` (`env_bool` / `env_int` / `env_float` / `env_str`), which read `os.environ` when called — usually once at module import to produce a module-level constant. This follows the existing `RLLM_HOME` / `RLLM_LOG_LEVEL` / `RLLM_SYMPY_TIMEOUT_S` precedent.

<Note>
  `env_bool`, `env_int`, and `env_float` raise `ValueError` on a malformed value — a typo in an ops knob fails loudly rather than silently falling back to the default.
</Note>
...
| `RLLM_LOOP_HEALTH_ENABLED` | `false` | Emit trainer and model-gateway event-loop health probes |
