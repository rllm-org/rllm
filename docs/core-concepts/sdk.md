...
**That's it!** Pass this function to `AgentTrainer` and training begins.
...
### 3. Multi-Step Agents with `@trajectory`

For agents with multiple LLM calls where you want per-step rewards:
...
Each LLM call produces a `Trace` containing:

- `prompt_token_ids` - Exact token IDs sent to the model

- `completion_token_ids` - Exact token IDs generated by the model  

- `logprobs` - Per-token log probabilities (when enabled)

- `messages` - Original message format

- `metadata` - Session metadata, timestamps, latency

### Trajectories

A trajectory is a sequence of steps (LLM calls) that form a complete agent interaction:

- **Single-step**: One LLM call per task (e.g., simple Q&A)

- **Multi-step**: Multiple calls with intermediate reasoning (e.g., ReAct, search agents)

- **Hierarchical**: Nested trajectories for sub-agents
