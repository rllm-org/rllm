...
A *harness* is a built-in [AgentFlow](/core-concepts/agentflow-evaluator): a generic agent program parameterised by the LLM and the task, with no per-benchmark code. rLLM ships three of them. The CLI's `--agent` flag picks one by name; the [Runner](/core-concepts/task-and-runner) drives it like any other AgentFlow.
...
| `openhands` | `rllm.harnesses.openhands.OpenHandsHarness` | Run All Hands AI's OpenHands inside the sandbox. |
| `swe-agent` | `rllm.harnesses.swe_agent.SweAgentHarness` | Run Princeton's SWE-agent inside the sandbox. |
| `pi` | `rllm.harnesses.pi.PiHarness` | Run Mario Zechner's pi-coding-agent CLI inside the sandbox. |
