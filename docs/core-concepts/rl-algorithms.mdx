...
Advantages are computed on `TrajectoryGroup`s, partitioned by `group_role`, before conversion to a backend batch. The normal estimator hook is **scalar reward per trajectory in, scalar advantage per trajectory out**; the collector immediately materializes that scalar as one target per `Step.response_ids` token. `minus_length_weighted_mean` can instead materialize distinct targets for those tokens.
...
| **Minus length-weighted mean** | `minus_length_weighted_mean` | Subtract a role-pool baseline weighted by response length; uses explicit token targets when every step supplies `metadata["per_token_rewards"]`. |
| **ECHO** | `echo` | GRPO's advantage **unchanged**, plus an environment-observation auxiliary loss (see below). |

All seven live in one registry and run on every backend. (The verl backend can also be pointed at its native estimators, but the rLLM-native set above is the portable path.)
